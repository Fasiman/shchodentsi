...
const ProfileDetail = ({ onLogoutClick }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
        if (file.size > 5 * 1024 * 1024) {
            alert('Файл занадто великий. Будь ласка, виберіть зображення менше 5МБ.');
            return;
        }
  ...
  const handleSave = async (e) => {
    ...
    try {
        ...
        await axios.put(`https://696f45bda06046ce6185fca4.mockapi.io/users/${currentUser.id}`, updatedUser);
          ...
          <div className="profile__photo">
            ...
            <div className="profile__photo-load">
              <div 
                  ...
                  style={{ backgroundImage: `url(${avatarPreview || currentUser.avatar})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
              ></div>
              <input 
                  type="file" 
                  ref={fileInputRef} 
                  ...
                  accept="image/*"
              />
              <button className="profile__photo-button" onClick={handleUploadClick}>
                Завантажити фото
              </button>
            </div>
          </div>
